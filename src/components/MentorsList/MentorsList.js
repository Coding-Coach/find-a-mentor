...
const tagsList = tags =>
  tags.map((tag, index) => {
    return (
      <Label className="mentor-tag" key={index}>
        {tag}
      </Label>
    );
  });

const items = (mentors, favs, onToggleFav) =>
  mentors.map((mentor, index) => {
    return (
      <Card className="mentor-card" key={index}>
        <header>
          <Label
            as="button"
            corner="right"
            onClick={onToggleFav.bind(null, mentor)}
          >
            <Icon
              name="heart"
              color={favs.indexOf(mentor.id) > -1 ? 'red' : 'black'}
            />
          </Label>
        ...
        <Button.Group attached="bottom">
          {mentor.channels.map(channel => {
            ...
            return (
              <a
                className="ui black basic button channel-link"
                key={channel.type}
                href={url}
                target="_blank"
...
export default class MentorsLists extends Component {
  ...
  render() {
    ...
    return (
      <div className={classNames(['mentors-wrapper', className])}>
